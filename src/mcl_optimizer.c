...
static void initialize_sgd_m (mcl_optimizer *opt)
{
    mcl_network *net = opt -> net;
...
static void calculate_grad_sgd_m (mcl_network *net, mcl_tensor **m,
                                  float beta1,
                                  float learn_rate, int batch_size)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        for (int j = 0; j < w_length; j++) {
            ...
            wg -> ten[j] = -learn_rate * wm -> ten[j]; 
        }
        for (int j = 0; j < b_length; j++) {
            bg -> ten[j] /= batch_size;
            bm -> ten[j] = beta1 * bm -> ten[j] + (1 - beta1) * bg -> ten[j];
            bg -> ten[j] = -learn_rate * bm -> ten[j]; 
        }
    }
}

static void train_batch_sgd_m (mcl_optimizer *opt, mcl_tensor **batch, int batch_size)
{
    mcl_network *net = opt -> net;
    mcl_tensor *output = net -> layers[net -> num_layers - 2] -> output;
    mcl_tensor *output_grad = net -> layers[net -> num_layers - 2] -> output_grad;
    ...
    calculate_grad_sgd_m (net, opt -> m, 
                         beta1,
                         learn_rate, batch_size);
    mcl_network_apply_grad (net);
}

void mcl_optimizer_train_sgd_m (mcl_optimizer *opt, int batch_size, int num_epochs)
{
    initialize_sgd_m (opt);
    ...
    for (int i = 0; i < num_epochs; i++) {
        ...
        for (int j = 0; j < train_size; j += batch_size) {
            int b_size = train_size - j < batch_size ? train_size - j : batch_size;
            train_batch_sgd_m (opt, &(dataset_train[j * 2]), b_size);
        }
    }
}

static void initialize_rmsprop (mcl_optimizer *opt)
{
    mcl_network *net = opt -> net;

    if ((net == NULL) || (opt -> v != NULL))
        return;
...
static void calculate_grad_rmsprop (mcl_network *net, mcl_tensor **v,
                                    ...
                                    float learn_rate, int batch_size)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        for (int j = 0; j < w_length; j++) {
            ...
            wg -> ten[j] *= -learn_rate / (sqrt (wv -> ten[j]) + epsilon); 
        }
        for (int j = 0; j < b_length; j++) {
            bg -> ten[j] /= batch_size;
            bv -> ten[j] = beta2 * bv -> ten[j] + (1 - beta2) * pow (bg -> ten[j], 2);
            bg -> ten[j] *= -learn_rate / (sqrt (bv -> ten[j]) + epsilon); 
        }
    }
}

static void train_batch_rmsprop (mcl_optimizer *opt, mcl_tensor **batch, int batch_size)
{
    mcl_network *net = opt -> net;
    mcl_tensor *output = net -> layers[net -> num_layers - 2] -> output;
    mcl_tensor *output_grad = net -> layers[net -> num_layers - 2] -> output_grad;
    ...
    calculate_grad_rmsprop (net, opt -> v, 
                            beta2, epsilon,
                            learn_rate, batch_size);
    mcl_network_apply_grad (net);
}

void mcl_optimizer_train_rmsprop (mcl_optimizer *opt, int batch_size, int num_epochs)
{
    initialize_rmsprop (opt);
    ...
    for (int i = 0; i < num_epochs; i++) {
        ...
        for (int j = 0; j < train_size; j += batch_size) {
            int b_size = train_size - j < batch_size ? train_size - j : batch_size;
            train_batch_rmsprop (opt, &(dataset_train[j * 2]), b_size);
        }
    }
}
...
static void initialize_adam (mcl_optimizer *opt)
{
    ...
    if ((net == NULL) || ((opt -> m != NULL) && (opt -> v != NULL)))
        return;
